...
//região do tabuleiro
let vBoard = []; // tabuleiro virtual para verificações
let turnPlayer = '';// jogador da vez

//função para mostrar jogador da vez: 
function updateTitle() { 
    const playerInput = document.getElementById(turnPlayer);
    document.getElementById('turnPlayer').innerText = playerInput.value;
};

// função de inicializar/preparar o jogo
...
function handleBoardClick(ev) {
    ...
    const rowColumnPair = region.split('.'); // ["N", "N"] Split transforma uma string em array. A condição aplicada para fazer isso é o ponto.
    ...
    console.clear(); //clear já é um método disponível no console
    console.table(vBoard); //table pega um dado e mostra como tabela
    ...
    } else if (vBoard.flat().includes('')) { //método flat retorna novo array com todos os sub-arrays, concatenando em um único array
        turnPlayer = turnPlayer === 'player1' ? 'player2' : 'player1';
        // se jogador atual for igual ao player 1, o próximo será o 2. senão, será o 1.
        updateTitle();
    } else {
